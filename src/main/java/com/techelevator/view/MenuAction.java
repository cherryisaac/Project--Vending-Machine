...
public class MenuAction extends Menu {//child of Menu
    ...
    private Inventory inventory = new Inventory(fileReader);
    ...
    public void displayInventory() {
        for (Map.Entry<String, Item> entry : inventory.itemMap.entrySet()) {
            Item item = entry.getValue();
            String formattedString = String.format("%-5s %-22s %-5s %-10s", entry.getKey(), item.getName(),
                    item.getPriceAsString(), item.getQuantity()  > 0 ? item.getQuantity() : "Sold Out");
    ...
    public void feedMoney() {
        ...
        try {
            ...
            if (moneyInserted == 1 || moneyInserted  == 2 || moneyInserted == 5 || moneyInserted == 10
                ...
                System.out.println("Thank You For inserting " + String.format("$%.2f", moneyInserted) + "  Total Money Available : "+displayCurrentBalance());
            } else {
